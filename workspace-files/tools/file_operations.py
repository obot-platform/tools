...
from tools.gptscript_workspace import (
...
async def read_file(input_file: str, max_file_size: int = MAX_FILE_SIZE):
    ...
    if len(tokens) > TOKEN_THRESHOLD:
        print(
            f"The uploaded file {input_file} contains too many tokens ({len(tokens)}), summarizing...\n"
        )
        ...
        response_str = f"Here is the summary of the file content:\n\n{final_summary}"
        print(response_str)
...
async def input_handler():
    ...
    if data.explain:
        print(
            f'Explain the following selection from the "{data.explain.filename}" workspace file: {in_back_ticks(data.explain.selection)}'
        )

    if data.improve:
        if data.improve.selection == "":
            print(
                f'Referring to the workspace file "{data.improve.filename}", {data.prompt} \nWrite any suggested changes back to the file'
            )
        else:
            print(
                f'Referring to the selection from "{data.improve.filename}", {data.prompt}: {in_back_ticks(data.improve.selection)} \nWrite any suggested changes back to the file'
            )
        ...
        for filename, content in data.changedFiles.items():
            if await write_file_in_workspace(filename, content):
                if not printed:
                    printed = True
                    print(
                        "The following files have been externally changed in the workspace, re-read them if the up to date content needs to be known:"
                    )
