...
async def load_text_from_file(file_path: str) -> str:
    ...
    if not file_path.endswith(SUPPORTED_TEXT_FILE_TYPES):
        raise ValueError(
            f"Error: the input file must end with one of the following file types: {SUPPORTED_TEXT_FILE_TYPES}, other file types are not supported yet."
        )
