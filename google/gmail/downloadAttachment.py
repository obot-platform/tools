...
class ObotClient:
    ...
    def upload_knowledge_file(
        self, project_id, thread_id, assistant_id, filename, content
    ):
...
def main():
    ...
    try:
        ...
        obot_client.upload_file(
            project_id,
            os.getenv("OBOT_THREAD_ID"),
            os.getenv("OBOT_AGENT_ID"),
            filename,
            attachment_data,
        )
        obot_client.upload_knowledge_file(
            project_id,
            os.getenv("OBOT_THREAD_ID"),
            os.getenv("OBOT_AGENT_ID"),
            filename,
        ...
        print(json.dumps({"error": f"Failed to retrieve attachment: {e}"}))
