...
func FetchUserProfile(ctx context.Context, accessToken string) (*githubUserProfile, error) {
	result, err := makeGitHubRequest[githubUserProfile](ctx, accessToken, "user")
	if err != nil {
		return nil, err
	}
	return &result, nil
...
func FetchUserGroupInfos(ctx context.Context, accessToken string) (state.GroupInfoList, error) {
	...
	orgs, err := makeGitHubRequest[[]githubOrganization](ctx, accessToken, "user/orgs")
	if err != nil {
	...
	teams, err := makeGitHubRequest[[]githubTeam](ctx, accessToken, "user/teams")
	if err != nil {
...
func makeGitHubRequest[T any](ctx context.Context, accessToken, path string) (T, error) {
	var result T
	...
	if resp.StatusCode != http.StatusOK {
		...
		return result, fmt.Errorf("unexpected status code: %d: %s", resp.StatusCode, body)
	}
